 ...
 *   --map                   no full code: signatures for every code file (a map of the repo)
 *   --tree-only             code outside the focus appears only in the file tree, with its exported
 *                           names (the most compact map; for very large repos)
 *
 * Example:
...
const IGNORED_DIRS = new Set([
  ...
  '.turbo', '.vite', '.cache', 'out', 'storybook-static', '__snapshots__', '.ai',
]);
...
const IGNORED_PATTERNS = [/\.min\.(js|css)$/, /\.map$/, /\.snap$/, /^tools\/ai\//];
...
const parseArgs = (argv: string[]): Options => {
  const opts: Options = { focus: [], depth: 1, sigDepth: Infinity, task: '', diff: true, out: 'project_context.txt', maxFileKb: 150, map: false, treeOnly: false };
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (flag) {
      ...
      case '--tree-only': opts.treeOnly = true; break;
...
/** Names a file exports (functions, components, consts, classes, types). */
const exportedNames = (text: string): string[] => {
  const names = new Set<string>();
  const re = /\bexport\s+(?:declare\s+)?(?:default\s+)?(?:async\s+)?(?:abstract\s+)?(?:function\*?|const|let|var|class|interface|type|enum)\s+([A-Za-z_$][\w$]*)/g;
...
const gitSection = (includeDiff: boolean, outFile: string): { text: string; redactions: number } => {
  const branch = git(['rev-parse', '--abbrev-ref', 'HEAD']);
  if (!branch) return { text: '', redactions: 0 };

  const log = git(['log', '--oneline', '-n', '10']);
  // Only diff files that are allowed in the pack (skips lockfiles, .env, tool files, …).
  const changed = includeDiff
    ? git(['diff', 'HEAD', '--name-only', '-z']).split('\0').map(toPosix).filter((f) => f && isEligible(f, outFile))
    : [];
  let diff = changed.length ? git(['diff', 'HEAD', '--', ...changed]) : '';
...
- <file_tree>: every file in the repo. [full] = complete source below, [sig] = signatures only, [exports: …] = names the file exports (source not included), unmarked = not included.
...
const packProject = (): void => {
  ...
  const focusing = opts.focus.length > 0 || opts.map;
  console.log(`Packing ${ROOT}${opts.focus.length ? ` (focus: ${opts.focus.join(', ')})` : opts.map ? ' (map: signatures only)' : ''}\n`);
  ...
  const sigScope = Number.isFinite(opts.sigDepth) && seedList.length ? neighbourhood(seedList, Math.max(opts.sigDepth, opts.depth), graph) : null;

  if (opts.focus.length > 0 && seeds.size === 0) {
  ...
  const modeOf = (rel: string): Mode => {
    ...
    return CODE_FILE.test(rel) && !opts.treeOnly && (!sigScope || sigScope.has(rel)) ? 'signatures' : 'tree-only';
  };
  ...
  const gitState = gitSection(opts.diff, outFile);
  ...
  // Tree-only code files: list their exported names so the tree still works as a map.
  for (const rel of packable) {
    if (marks.has(rel) || !CODE_FILE.test(rel)) continue;
    const names = exportedNames(texts.get(rel) ?? '');
    if (names.length) marks.set(rel, `exports: ${names.slice(0, 6).join(', ')}${names.length > 6 ? ', …' : ''}`);
  }
  const projectName = (() => {
